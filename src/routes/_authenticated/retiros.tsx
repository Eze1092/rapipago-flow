...
function Retiros() {
  ...
  const alta = useMutation({
    mutationFn: async () => {
      ...
      // Las bocas son opcionales. Solo se crean relaciones si el usuario eligió alguna.
    ...
    <AppLayout titulo="Retiros del camión">
      {esAdmin && (
        <Panel titulo="Registrar retiro">
          <form className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" onSubmit={(e) => { e.preventDefault(); if (!window.confirm(`¿Confirmás el retiro de ${formatARS(parseImporte(form.retirado))}?`)) return; alta.mutate(); }}>
            ...
            <div className="sm:col-span-2 lg:col-span-3"><span className="label-xs text-muted-foreground">Bocas incluidas (opcional)</span><div className="mt-2 flex flex-wrap gap-2"><button type="button" className={form.bocas.length === 0 ? "rounded-lg bg-ink px-3 py-1.5 font-mono text-xs uppercase tracking-wide text-cream" : "btn-ghost"} onClick={() => setForm({ ...form, bocas: [] })}>Sin boca</button>{(bocas.data ?? []).map((b) => { const elegida = form.bocas.includes(b.id); return <button type="button" key={b.id} onClick={() => setForm({ ...form, bocas: elegida ? form.bocas.filter((x) => x !== b.id) : [...form.bocas, b.id] })} className={elegida ? "rounded-lg bg-ink px-3 py-1.5 font-mono text-xs uppercase tracking-wide text-cream" : "btn-ghost"}>{b.codigo}</button>; })}</div></div>
            <div className="sm:col-span-2 lg:col-span-4"><button className="btn-primary" disabled={alta.isPending}>{alta.isPending ? "Registrando…" : "Registrar retiro"}</button></div>
          </form>
        </Panel>
      )}
      <Panel titulo="Retiros registrados" extra={<span className="label-xs text-muted-foreground">{filas.length} registros</span>}>
        <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="label-xs text-muted-foreground"><th className="px-3 py-2 text-left font-normal">Fecha</th><th className="px-3 py-2 text-left font-normal">Hora</th><th className="px-3 py-2 text-left font-normal">Bocas</th><th className="px-3 py-2 text-left font-normal">Remito</th><th className="px-3 py-2 text-right font-normal">Declarado</th><th className="px-3 py-2 text-right font-normal">Retirado</th><th className="px-3 py-2 text-right font-normal">Acreditado</th><th className="px-3 py-2 text-right font-normal">Estado</th></tr></thead><tbody className="divide-y divide-border">{filas.map((r) => { const acreditado = (r.acreditaciones ?? []).filter((a) => a.estado !== "ANULADO").reduce((acc, a) => acc + Number(a.importe), 0); return <tr key={r.id} className="transition hover:bg-ink/5"><td className="num px-3 py-2.5">{formatFecha(r.fecha)}</td><td className="num px-3 py-2.5">{formatHora(r.hora)}</td><td className="px-3 py-2.5">{(r.retiro_bocas ?? []).map((rb) => rb.bocas?.codigo).filter(Boolean).join(" + ") || "Sin boca"}</td><td className="px-3 py-2.5">{r.remito || "-"}</td><td className="num px-3 py-2.5 text-right">{formatARS(r.importe_declarado)}</td><td className="num px-3 py-2.5 text-right">{formatARS(r.importe_retirado)}</td><td className="num px-3 py-2.5 text-right">{formatARS(acreditado)}</td><td className="px-3 py-2.5 text-right"><Tag estado={r.estado} texto={etiquetaEstado[r.estado] ?? r.estado} /></td></tr>; })}</tbody></table>{filas.length === 0 && <EstadoVacio texto="Todavía no se registraron retiros." />}</div>
      </Panel>
    </AppLayout>
  );
}
